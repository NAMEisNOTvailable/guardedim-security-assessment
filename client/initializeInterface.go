...
// createWG0Client creates a userspace WireGuard interface for a client.
// No ListenPort is set; outbound UDP uses an ephemeral port.
...
// Configure the client interface IP/MTU and bring the link up.
...
// InitializeInterface creates wg0, configures IP/MTU, and returns the device
// so the caller can add peers later.
