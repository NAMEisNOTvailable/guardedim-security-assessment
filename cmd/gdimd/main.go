...
func main() {
	...
	case "client":
		...
		if err != nil {
			fmt.Printf("Could not open the local database: %v", err)
			os.Exit(1)
		}
		...
		g.Go(func() error {
			wgDev, err := client.InitializeInterface(wg_privip, privkey, wg_MTU)
			if err != nil {
				fmt.Printf("Could not start the WireGuard interface: %v", err)
				return err
			}
			<-ctx.Done()
			wgDev.Close()
		...
		if err := g.Wait(); err != nil {
			log.Fatalf("gdimd stopped: %v", err)
		}
	...
	case "server":
		...
		// Start WireGuard and stop it when the daemon context is cancelled.
		g.Go(func() error {
		...
		// Start the mTLS control server with the same shutdown context.
		g.Go(func() error {
