...
func UpdateConnection(db *sql.DB) error {
	...
	// Preserve local interface settings while replacing the peer set from the database.
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.PubIP, &row.Port, &row.PrivIP, &row.PubKey, &row.PSK); err != nil {
			log.Printf("Could not read server row: %v", err)
			continue
		}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&pubKeyBytes, &userIP); err != nil {
			log.Printf("Could not read user row: %v", err)
			continue
		}
